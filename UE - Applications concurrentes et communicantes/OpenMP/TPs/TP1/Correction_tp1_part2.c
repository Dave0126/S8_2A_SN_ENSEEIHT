...
/* This routine performs a parallel LU factorization based on the
   ...
   CC: all the threads share the progress table. This means that the
     acces to the progress table (both in read and write) has to be
     protected in order to avoid multiple threads to access it at the
     same time. A critical section is the easiest way of doing this; a
     better solution is to use a lock.
...
#pragma omp critical
      ...
      switch(task.type)
        ...
        case PNL:
          ...
          /* CC: the access to the progress table has to be protected
             by a lock or a critical section. Invite the students to
             think about the difference between the two and the
             advantages of using a lock instead of a critical
             section. */
#pragma omp critical
          {
