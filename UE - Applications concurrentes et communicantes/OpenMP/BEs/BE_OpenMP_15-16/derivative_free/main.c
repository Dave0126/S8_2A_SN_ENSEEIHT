...
#pragma omp parallel for private(nx,ny,nz) num_threads(4)
    for(i=0; i<p; i++){
      nx = x+ s*cos(2.0*M_PI*i/((double)p));
      ny = y+ s*sin(2.0*M_PI*i/((double)p));
      nz = evaluate(nx,ny);
...
Test result:
./main 10240 0.01
Minimum found is 1.5670285145  at x=0.6959, y=0.1332  in 1373 steps
Sequential   time    :   255.76 msec.
Parallel     time    :   250.06 msec. (1 thread)
Parallel     time    :   134.56 msec. (2 threads)
Parallel     time    :    98.53 msec. (4 threads)

通过实验结果我们可知，双线程的效率基本上是单线的两倍；
四线程由于多线程额外的系统开销，虽然没有达到理论值，但依旧符合预期。
*/