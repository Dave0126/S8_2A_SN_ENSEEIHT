...
let meet x y = match x, y with
  |Itv (n1, n2), Itv (n3, n4) -> if ((min_minf n2 n3) = n2 || (min_minf n1 n4) = n4) then bottom else Itv (max_pinf n1 n3, min_minf n2 n4)
...
let order x y = match x, y with
  | Bot, _ -> true
  | _, Bot -> false
  | Itv (n1, n2), Itv (n3, n4) -> if n1 <= n3 then true else false

let widening x y = join x y 
