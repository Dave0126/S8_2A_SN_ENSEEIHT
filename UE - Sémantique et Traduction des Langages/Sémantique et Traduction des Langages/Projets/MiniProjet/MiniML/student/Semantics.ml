...
ruleRead _env _expr mem = 
    (let (vexpr,m2) = (value_of_expr(_expr,mem) _env) in
    (match vexpr with
      | (ErrorValue _) as result -> (result,m2)
      | ReferenceValue reference -> 
        (match (lookforMem reference m2) with 
        | NotFound -> ((ErrorValue (UnknownReferenceError reference)), m2)
...
ruleWrite _env _refexpr _valexpr mem = 
        (let (vrefexpr,m2) = (value_of_expr (_refexpr, mem) _env)
      in (match vrefexpr with 
      | (ErrorValue _) as result -> (result,m2)
      | ReferenceValue reference -> 
        (match (lookforMem reference m2) with 
        | NotFound -> ((ErrorValue (UnknownReferenceError reference)), m2)
        | Found _ -> (let (vvalexpr, mvalexpr) = (value_of_expr (_valexpr, m2) _env)
      in (match vvalexpr with 
          | (ErrorValue _) as result -> (result,mvalexpr)
          | _ -> (NullValue,((reference,vvalexpr)::mvalexpr)))))
      | _ -> ((ErrorValue TypeMismatchError),m2)))
...
ruleSequence _env _left _right mem = (let (leftvalue,leftmem) = (value_of_expr (_left,mem) _env) in
      (match leftvalue with 
      | (ErrorValue _ ) as result -> (result, leftmem)
      | NullValue -> (value_of_expr (_right,leftmem) _env)
      | _ -> ((ErrorValue TypeMismatchError),leftmem)))
...
ruleWhile _env _cond _body mem = (let (vcond, m2) = (value_of_expr(_cond, mem) _env) in 
      (match vcond with 
      | (BooleanValue true) -> let (v,m3) = (value_of_expr(_body,m2) _env) in
        (match v with 
        | NullValue -> ruleWhile _env _cond _body m3  
...
ruleReference _env _expr mem = (let (vexpr, m2) = (value_of_expr (_expr,mem) _env)
    in 
    (match vexpr with
    | (ErrorValue _) as result -> (result,m2)
    | result -> 
      (let reference = newReference() in 
      ((ReferenceValue reference),((reference,result)::m2))))) 
