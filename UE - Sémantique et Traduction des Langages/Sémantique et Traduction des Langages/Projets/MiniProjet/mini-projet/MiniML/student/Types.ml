...
let rec type_of_expr expr env =
  ...
  and
    ...
    ruleFunction _env _par _body = 
      let t_par = newVariable () in
        let t_body = (type_of_expr _body ((_par,t_par)::_env)) in
          FunctionType (t_par,t_body)


  and
    (* ...............A COMPLETER .......................................*)
    ruleCall _env _fct _par =
      let t_func = type_of_expr _fct _env in 
        let ut, ur = unify t_func (FunctionType ((type_of_expr _par _env), newVariable())) in
        match ut with 
        | FunctionType (_, t_res) -> if ur then t_res else ErrorType
        | _ -> ErrorType
  ...
  and
    ruleRef _env _expr = let t = type_of_expr _expr _env in 
    ReferenceType(t)

  and
    (* ...............A COMPLETER .......................................*)
    ruleRead _env _expr = let t = type_of_expr _expr _env in 
    match t with 
      | ReferenceType(t) -> t
      | _ -> ErrorType
  and
    (* ...............A COMPLETER .......................................*)
    ruleWrite _env _left _right = let t = type_of_expr _left _env in 
    match t with 
      | ReferenceType(t) -> let t2 = type_of_expr _right _env in 
      let _,ur = unify t t2 in if ur then UnitType else ErrorType
      | _ -> ErrorType

  and
    (* ...............A COMPLETER .......................................*)
    ruleSequence _env _left _right = let t = type_of_expr _left _env in 
    match t with 
    | UnitType -> type_of_expr _right _env 
    | _ -> ErrorType

  and
    ruleWhile _env _cond _body = let t = type_of_expr _cond _env in 
    match t with 
    | BooleanType -> let t2 = type_of_expr _body _env in 
    let _,ur = unify UnitType t2 in if ur then UnitType else ErrorType
    | _ -> ErrorType
